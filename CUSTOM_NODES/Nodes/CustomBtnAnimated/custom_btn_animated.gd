...
@export var is_pressed : = false							:set = set_is_pressed
...
func set_text(te : String):
	$MarginContainer/InfoBox/Text.set_text(te)
	text = te

func set_icon(ico : Texture):
	$MarginContainer/InfoBox/Icon.set_texture(ico)
	icon = ico

func set_is_pressed(is_p : bool):
	is_pressed = is_p
